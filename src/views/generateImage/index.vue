...
import generateImage from "@/api/qfanApi.ts"
import { addFavoriteImage, removeFavoriteImage, updateImageCount } from "@/api/imageApi"
    ...
    setup(props) {
        ...
        async function handleGenerate() {
            ...
            try {
                ...
                if (res.error_code) {
                    ...
                    snackbarStore.showErrorMessage('生成失败-' + res.error_msg)
                }
                else {
                    imageUrls.value = res.data
                    snackbarStore.showSuccessMessage('生成成功')
                }
            } catch (error) {
                loading.value = false
                console.log(error, 'handleGenerateFalse');
                snackbarStore.showErrorMessage('生成失败-' + error)
            }
        }
        ...
        async function handleFavorite(item) {
            ...
            try {
                ...
                if(res.code === 200) {
                    snackbarStore.showSuccessMessage(item.id ? '取消收藏' : '收藏成功')
                ...
                snackbarStore.showErrorMessage(item.id ? '取消失败' : '收藏失败')
            }
        }
