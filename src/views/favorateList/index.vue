    ...
    <div class="image-list">
        ...
        >
            ...
            <SvgIcon
                ...
                :icon-class="item.imageId ? 'favorite-filling' : 'favorite-empty'"
            ></SvgIcon>
        </div>
    </div>
    <v-dialog
...
import { getFavoriteImagesByUserId, removeFavoriteImage, addFavoriteImage, publishImgToSociety } from '@/api/imageApi'
    ...
    setup() {
        ...
        async function handleFavorite(item) {
            ...
            try {
                ...
                if(res.code === 200) {
                    snackbarStore.showSuccessMessage(item.imageId ? '取消收藏' : '收藏成功')
                ...
                snackbarStore.showErrorMessage(item.imageId ? '取消失败' : '收藏失败')
            }
        }
        ...
        async function getFavoriteImageList() {
            try {
                if(authStore.id) {
                    const res = await getFavoriteImagesByUserId(authStore.id)
                    if(res.code === 200) {
                        imageUrls.value = res.data
                        snackbarStore.showSuccessMessage('获取收藏列表成功')
                    }
                }
            } catch (error) {
                console.log(error);
        ...
        async function handlePublish() {
            const param = {
                ...
                imageId: focusedItem.value.imageId,
            }
            try {
                ...
                if(res.code === 200) {
                    ...
                    snackbarStore.showErrorMessage(res.message || '发布失败')
                }
            } catch (err) {
                console.log(err)
            }
